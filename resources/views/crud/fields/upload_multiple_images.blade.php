...
@if (isset($field['value']))
    ...
    @endphp
    @if (count($values))
        <div class="well well-sm existing-file row popup-gallery">
            @foreach($values as $key => $file_path)
                <div class="col-lg-2">
                    <div class="card">
                        @if (isset($field['temporary']))
                            <a data-lightbox="imagegroup" target="_blank" href="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->temporaryUrl($file_path, Carbon\Carbon::now()->addMinutes($field['temporary']))):asset($file_path) }}">{{ $file_path }}</a>
                        @else
                            <a data-lightbox="imagegroup" target="_blank" href="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->url($file_path)):asset($file_path) }}">
                                <img  class="card-img-top img-fluid" src="{{ isset($field['disk'])?asset(\Storage::disk($field['disk'])->url($file_path)):asset($file_path) }}" alt="">
                            </a>
                        @endif
                        <div class="card-body">
                            <a href="#" class="btn btn-light btn-sm float-right file-clear-button" title="Clear file" data-filename="{{ $file_path }}"><i class="bx bx-trash"></i></a>
                        </div>
                    </div>
                </div>
            @endforeach
...
<div class="backstrap-file mt-2">
    <input
        ...
        @include('skote::crud.fields.inc.attributes', ['default_class' =>  isset($field['value']) && $field['value']!=null?'file_input backstrap-file-input':'file_input backstrap-file-input'])
        multiple
    >
